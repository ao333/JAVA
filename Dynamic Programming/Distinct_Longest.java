...
public class Distinct_Longest {
    ...
    /* 1a) The worst case time complexity of this solution is O(N^2) since there is a nested for-loop. While j=i+1 can
    reduce the number of iterations for the second loop, both are still proportional to the array size, so the overall
    time complexity does not change. The space complexity is O(1), since only i and j are created.
    ...
    static boolean DISTINCT(int a[]) {
        for (int i=0; i<a.length-1; i++)
            for (int j=i+1; j<a.length; j++)
                if (a[i] == a[j]) return false;
        return true;
    }
    /* 1b) Distinct uses a Hash Set. The structure, which is similar to a hash table but disallows duplicate keys, was
    chosen for its suitability to this question. The contains method check against a set in constant time. The add method
    builds that set in amortized O(1) time because it may need to rehash and enlarge the table. However, since we still
    need to iterate through the list to compare and build the list, the overall function is still proportional to the
    array size, therefore O(N). While the single for-loop and hashing reduces time complexity, the creation of a new hash
    increases its space complexity to O(N). This is the trade-off compared to implementations in 1 a). */
    static boolean DISTINCT2(int a[]) {
    ...
    static int LONGEST(int a[]) {
        ...
        int max = 0;
        for (int x=0; x<size; x++) longest[x] = 1;
        for (int i=1; i<size; i++)
            for (int j=0; j<i; j++)
                if (a[i]>a[j] && longest[i]<longest[j]+1) longest[i] = longest[j] + 1;
        for (int y=0; y<size; y++) if (max < longest[y]) max = longest[y];
        return max;
    }
}
